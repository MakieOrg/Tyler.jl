        ...
        for (tile, data) in downloaded_tiles
            ...
            try
                if isnothing(data)
                    ...
                    # Also remove from foreground_tiles so wait() doesn't wait forever for failed tiles
...
function Base.display(map::AbstractMap)
    # Don't wait here - just display the figure
    # This prevents race condition with AMD drivers where plots are inserted during display
    # Users can call wait(map) explicitly if they need to wait for all tiles to load
