...
whale = CSV.read("./docs/examples/data/whale_shark_128786.csv", DataFrame)
lon = whale[!, :lon]
lat = whale[!, :lat]
steps = size(lon,1)
...
m = Tyler.Map(Rect2f(lomn - δlon/2, lamn-δlat/2, 2δlon, 2δlat), 5;
...
record(m.figure, joinpath("assets", "whale_shark_128786.mp4"),
